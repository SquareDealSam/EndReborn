#!/usr/bin/env python3
"""Generate 1.21.4+ item-model definitions (assets/endreborn/items/<name>.json).
...
each `items/<name>.json` simply references `endreborn:item/<name>`.
...
ITEM_MODELS = os.path.join(RES, "assets", "endreborn", "models", "item")
ITEM_DEFS = os.path.join(RES, "assets", "endreborn", "items")
...
def main():
    ...
    print(f"wrote {len(names)} item-model definitions to assets/endreborn/items/")
